export default function MobileMenuButton({ isOpen, ...props }) {
  ...
  const correctiveButtonStyle = isOpen ? "absolute right-2 top-2" : "";
  ...
  return (
    <button
      className={`z-50 w-10 h-8 flex flex-col justify-between before:bar ${correctiveEdgeBarStyle} after:bar ${correctiveButtonStyle} sm:hidden`}
